...
%If the position of the EE if within the bounds of an obstacle 
    if norm(pos(1,:) - obs(1,:)) <= rho
        ...
        F_rep = nu*((1/norm(origin_start-obs(1,:)')) - (1/rho))*(1/norm(origin_start-obs(1,:)')^2)*((origin_start - obs(1,:)')/norm(origin_start - obs(1,:)'));
    elseif norm(pos(1,:) - obs(2,:)) <= rho
        F_rep = F_rep + nu*((1/norm(origin_start-obs(2,:)')) - (1/rho))*(1/norm(origin_start-obs(2,:)')^2)*((origin_start - obs(2,:)')/norm(origin_start - obs(2,:)'));
    else
        F_rep = [0;0;0];
    end
